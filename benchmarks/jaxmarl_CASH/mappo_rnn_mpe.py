...
from jaxmarl.policies.gapnet_policies import ActorGapHyperRNN
import sys
sys.path.append("/homes/dkurtenb/projects/GapNet_MARL/code_repos/CASH/jaxmarl")
...
def make_train(config, viz_test_env, viz_train_eval_env):
    ...
    train_eval_env = MPEWorldStateWrapper(viz_train_eval_env)
    train_eval_env = MPELogWrapper(train_eval_env)
    ...
    def train(rng):
        # INIT NETWORK
        arch = config.get("AGENT_ARCH", "cash")
        if arch =="gapnet":
            actor_network = ActorGapHyperRNN(
                action_dim=env.action_space(env.agents[0]).n,
                hidden_dim=config["GRU_HIDDEN_DIM"],
                init_scale=config['AGENT_INIT_SCALE'],
                num_agents=config["ENV_KWARGS"]["num_agents"],
                num_capabilities=config["ENV_KWARGS"]["num_capabilities"],
                gap_func_dim=config.get("GAP_FUNC_DIM", 8),
                gap_phi_hidden=config.get("GAP_PHI_HIDDEN", 64),
                hypernet_kwargs=config['AGENT_HYPERNET_KWARGS'],
            )
        elif arch == "cash" or config.get("AGENT_HYPERAWARE", False):
        ...
        def _update_step(update_runner_state, unused):
            ...
            indist_results = _get_greedy_metrics(_rng2, train_states[0].params, eval_env=train_eval_env)
            test_metrics, viz_env_states = test_results["metrics"], test_results["viz_env_states"]
            metric["test_metrics"] = test_metrics
            metric["indist_metrics"] = indist_results["metrics"]
            # metric["test_metrics"] = {}
                        ...
                        wandb.log(
                            {
                                ...
                                **{'indist_'+k:v.mean() for k, v in metric['indist_metrics'].items()},
                            }
                        )
            ...
            jax.debug.callback(callback, metric, traj_batch.info)
            update_steps = update_steps + 1
            runner_state = (train_states, env_state, last_obs, last_done, hstates, viz_env_states, rng)
            return (runner_state, update_steps), metric

        def _get_greedy_metrics(rng, actor_params, eval_env=None):
            if eval_env is None:
                eval_env = test_env
            """
            ...
            def _greedy_env_step(step_state, unused):
                ...
                # thus, duplicate across all n_agents
                if "makespan" in info:
                    info["makespan"] = info["makespan"].reshape(-1, 1).repeat(eval_env.num_agents, axis=1)

                if "quota_met" in info:
                    info["quota_met"] = info["quota_met"].reshape(-1, 1).repeat(eval_env.num_agents, axis=1)
            ...
            rng, _rng = jax.random.split(rng)
            reset_rng = jax.random.split(_rng, config["NUM_ENVS"])
            init_obsv, env_state = jax.vmap(eval_env.reset, in_axes=(0,))(reset_rng)
            ...
            snd_value = snd(rollouts=snd_obs, hiddens=snd_hstate, dim_c=eval_env.num_agents*2, params=actor_params, alg='mappo', agent=actor_network)
...
def main(config):
    ...
    viz_train_eval_env = jaxmarl.make(config["ENV_NAME"], **config["ENV_KWARGS"])

    rng = jax.random.PRNGKey(config["SEED"])
    rngs = jax.random.split(rng, config["NUM_SEEDS"])
    train_vjit = jax.jit(jax.vmap(make_train(config, viz_test_env, viz_train_eval_env)))
